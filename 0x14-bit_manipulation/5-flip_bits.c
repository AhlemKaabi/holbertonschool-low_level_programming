...
unsigned int flip_bits(unsigned long int n, unsigned long int m)
{
	...
	else
	{
		...
		*	[ flipped & (flipped - 1) ] this comparation will return the flipped number without that first bit (= 1)
		*	and so on until the all bits that constitute the filpped bit number == 0
		* }
		*/
		/*count set bits of the result = flipped bit number. e.g : flipped = 0101001 --> 3*/
		while (flipped > 0)
		{
      		count += flipped & 1;
     		flipped >>= 1;
		/**
		* this methos is more simple it start to comapre the LSB of the flipped bit number
		...
		* now LSB = 1
		* until the flipped number becomes 000000000000...... according to the bit number size(8 bits / 32bits ...)
		*/
		}
		return (count);
	}
